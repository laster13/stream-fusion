...
class TheOldSchoolAPI:
    def __init__(
        ...
        self.base_url = (base_url or settings.theoldschool_url or "https://theoldschool.cc").rstrip("/")
    ...
    async def _search_all(self, params_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        if not self.api_key:
            logger.debug("TheOldSchool: No API key configured, skipping")
            return []
        if not params_list:
            return []
    ...
    async def _run_requests(
        ...
        for resp in responses:
            if isinstance(resp, Exception):
                logger.warning(f"TheOldSchool: Ignored failed search task: {resp}")
                continue
        ...
        try:
            async with session.get(url, params=params, headers=headers, timeout=15) as response:
                if response.status != 200:
                    logger.warning(f"TheOldSchool: HTTP {response.status}")
                    return []
