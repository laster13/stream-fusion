    ...
    def _is_stream_allowed(self, stream: TorrentItem) -> bool:
        ...
        if parsed_data.quality:
            ...
            if quality_upper in self.excluded_qualities:
                logger.trace(f"Stream excluded due to {parsed_data.quality} quality : {parsed_data.raw_title}")
                return False

        if parsed_data.resolution:
            resolution_upper = parsed_data.resolution.upper()
            if resolution_upper in self.excluded_qualities:
                logger.trace(f"Stream excluded due to quality spec ( {parsed_data.resolution} ): {parsed_data.raw_title}")
                return False
            if self.exclude_rips and resolution_upper in self.RIPS:
                logger.trace(f"Stream excluded due to RIP: {parsed_data.raw_title}")
                return False
            if self.exclude_cams and resolution_upper in self.CAMS:
                logger.trace(f"Stream excluded due to CAM: {parsed_data.raw_title}")
                return False

        if parsed_data.codec:
            codec_upper = parsed_data.codec.upper()
            if self.exclude_hevc and codec_upper == "HEVC":
                logger.trace(f"Stream excluded due to HEVC codec: {parsed_data.raw_title}")
                return False
